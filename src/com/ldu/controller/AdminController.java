...
//	}
	...
	public String getUserList(Model model,
		...
		int pageNum = page;
		int pageSize = rows;
		System.out.println("/n当前第几页"+pageNum);
		if (StringUtils.isEmpty(searchUser.getPhone())) {  //" "
			searchUser.setPhone(null);
		}
		if (StringUtils.isEmpty(searchUser.getUsername())) {
			searchUser.setUsername(null);
		}
		if (StringUtils.isEmpty(searchUser.getQq())) {
			searchUser.setQq(null);
		}
		
		PageInfo<User> userPage=this.userService.getPageUserByUser(pageNum,pageSize,searchUser);
		List<User> List=userPage.getList();
				System.out.println("分页后pageinfo中的值");
				for (User user : List) {
				...
				System.out.println("一页几条："+userPage.getPageSize());
				System.out.println("总共几页："+userPage.getPages());
...
//	
	...
	public String getGoodsList(Model model,
		...
		int pageSize = rows;
		System.out.println("/n当前第几页"+pageNum);
		if (StringUtils.isEmpty(searchGoods.getId())) {  //" "
			searchGoods.setId(null);
		}
		if (StringUtils.isEmpty(searchGoods.getName())) {
			searchGoods.setName(null);
		}
		if (StringUtils.isEmpty(searchGoods.getStatus())) {
			searchGoods.setStatus(null);
		}
		
		PageInfo<Goods> goodsPage=this.goodsService.getPageGoodsByGoods(pageNum,pageSize,searchGoods);
		List<Goods> List=goodsPage.getList();
				...
				System.out.println();
				System.out.println("总条数："+goodsPage.getTotal());
				System.out.println("当前页码："+goodsPage.getPageNum());
				System.out.println("一页几条："+goodsPage.getPageSize());
				System.out.println("总共几页："+goodsPage.getPages());
//		//获取顾客来源列表
//		List<BaseDict> fromType = this.basedictService.selectByCode("002");
//		List<BaseDict> industryType = this.basedictService.selectByCode("001");
//		List<BaseDict> levelType = this.basedictService.selectByCode("006");
//		model.addAttribute("fromType", fromType);
//		model.addAttribute("industryType",industryType);
//		model.addAttribute("levelType",levelType);
		model.addAttribute("goodsList", List);
		model.addAttribute("page", pageNum);
		model.addAttribute("total", goodsPage.getPages());
...
//	}
	...
	public String getOrdersList(Model model,
		...
		int pageSize = rows;
		System.out.println("/n当前第几页"+pageNum);
		if (StringUtils.isEmpty(searchOrders.getId())) {  //" "
			searchOrders.setId(null);
		}
		if (StringUtils.isEmpty(searchOrders.getOrderInformation())) {
			searchOrders.setOrderInformation(null);
		}
		if (StringUtils.isEmpty(searchOrders.getOrderState())) {
			searchOrders.setOrderState(null);
		}
		
		PageInfo<Orders> ordersPage=this.ordersService.getPageOrdersByOrders(pageNum,pageSize,searchOrders);
		List<Orders> List=ordersPage.getList();
				...
				System.out.println();
				System.out.println("总条数："+ordersPage.getTotal());
				System.out.println("当前页码："+ordersPage.getPageNum());
				System.out.println("一页几条："+ordersPage.getPageSize());
				System.out.println("总共几页："+ordersPage.getPages());

		model.addAttribute("ordersList", List);
		model.addAttribute("page", pageNum);
		model.addAttribute("total", ordersPage.getPages());
...
//	}
	...
	public String getPurseList(Model model,
			Purse searchPurse,
			@RequestParam(defaultValue="1")Integer page,
			@RequestParam(defaultValue="10")Integer rows
			){
		...
		int pageSize = rows;
		System.out.println("/n当前第几页"+pageNum);
		if (StringUtils.isEmpty(searchPurse.getUserId())) {  //" "
			searchPurse.setUserId(null);
		}
		if (StringUtils.isEmpty(searchPurse.getState())) {
			searchPurse.setState(null);
		}
		
		PageInfo<Purse> pursePage=this.purseService.getPagePurseByPurse(pageNum,pageSize,searchPurse);
		List<Purse> List=pursePage.getList();
				System.out.println("分页后pageinfo中的值");
				for (Purse purse : List) {
					System.out.println(purse);
				}
				System.out.println();
				System.out.println("总条数："+pursePage.getTotal());
				System.out.println("当前页码："+pursePage.getPageNum());
				System.out.println("一页几条："+pursePage.getPageSize());
				System.out.println("总共几页："+pursePage.getPages());

		model.addAttribute("purseList", List);
		model.addAttribute("page", pageNum);
		model.addAttribute("total", pursePage.getPages());
		model.addAttribute("searchpurse", searchPurse);
